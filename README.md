...
- **菜单栏常驻**（⚡）：状态指示（绿/黄/红）、打开主窗口、浏览器打开、刷新页面、启动/停止服务器、发送测试通知、设置、退出
- **设置**：端口、启动命令、自动启动服务器、退出时保持服务器、开机自启（SMAppService）
- **桌面桥接插件**（dsh-desktop-bridge）：`/api/desktop/status` + `/api/desktop/notify`，桌面集成本身就是一个 DSH 插件
...
│   MenuBarExtra 菜单栏    Settings 设置窗口               │
├─────────────────────────────────────────────────────────┤
│ Web 层（WebKit）                                         │
...
├─────────────────────────────────────────────────────────┤
│ 桥接层（dsh-desktop-bridge 插件，运行在 DSH 进程内）      │
│   GET  /api/desktop/status   服务器状态                  │
│   POST /api/desktop/notify   osascript 原生通知          │
└─────────────────────────────────────────────────────────┘
```
...
3. **菜单栏**（⚡）：快捷操作都在这里；关闭窗口不退出应用（菜单栏常驻），Cmd+Q 退出
